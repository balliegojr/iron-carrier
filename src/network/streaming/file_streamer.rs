...
use tokio::{io::AsyncRead, io::AsyncReadExt, io::AsyncWrite, io::AsyncWriteExt, io::{ReadHalf, WriteHalf}};
use crate::{config::Config, sync::file_events_buffer::FileEventsBuffer, fs::{self, FileInfo}};
...
impl <T:AsyncWrite + Unpin> Sender<T> {
    ...
    pub async fn send_file<R: AsyncRead + Unpin>(&mut self, ident: u64, buf_read: &mut R) -> crate::Result<()> {
...
impl <'a, T : AsyncRead + Unpin> Receiver<'a, T> {
    ...
    async fn read_file<'b>(&mut self, file_info: FileInfo, events_buffer: &'b FileEventsBuffer) -> crate::Result<()> {
    ...
    pub async fn wait_files<'b>(&mut self, events_buffer: &'b FileEventsBuffer) -> crate::Result<()> {
        while self.files.len() > 0 {
            ...
            match self.files.remove(&file_handle) {
                Some(file_info) => { self.read_file(file_info, events_buffer).await?; }
                None => { 
...
pub(crate) fn file_streamers<'a, T>(stream: T, config: &'a Config, peer_address: String) -> (Receiver<'a, ReadHalf<T>>, Sender<WriteHalf<T>>) 
...
mod tests {
    ...
    fn sample_config(test_folder: &str) -> Config {
        Config::parse_content(format!("port = 8090
    ...
    async fn file_streamer() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut file = FileInfo::new("a".into(), "file_1".into(), Path::new("./tmp/file_streamer/file_1").metadata().unwrap());
        ...
        assert_eq!(tokio::fs::read_to_string("./tmp/file_streamer/file_1").await.unwrap(), "some file content");
