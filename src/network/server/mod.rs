...
use tokio::{net::TcpListener, sync::Mutex, sync::mpsc::Sender, net::TcpStream};
...
use super::streaming::{file_streamers, get_streamers};
                    ...
                    Ok((stream, socket)) => { 
                        ...
                        if !handlers.contains_key(&socket_addr) {
                            ...
                            tokio::spawn(async move {
                                ...
                                let (file_receiver, file_sender) = file_streamers(file_stream, &config, socket_addr.clone());
                                ...
                                match handler.handle_events(sync_events, &file_events).await {
                                    Ok(()) => { log::info!("Peer connection closed: {}", socket_addr)}
