...
pub(crate) struct FileWatcherEventBlocker {
    config: Arc<Config>,
    events: RwLock<HashMap<PathBuf, (IpAddr, std::time::Instant)>>,
}
...
impl FileWatcherEventBlocker {
    ...
    pub fn allowed_peers_for_event(&self, file: &FileInfo) -> Option<Vec<SocketAddr>> {
        ...
        {
            if event_time > limit {
                peers.retain(|p| p.ip() != event_peer_address);
            }
        }
        Some(peers)
    }

    pub fn block_next_event(&self, absolute_path: PathBuf, peer_address: SocketAddr) {
        let mut received_events_guard = self.events.write().unwrap();
        received_events_guard.insert(
            absolute_path.clone(),
            (peer_address.ip(), std::time::Instant::now()),
        );
    }
}
...
mod tests {
    ...
    pub async fn filter_events() {
        ...
        buffer.block_next_event(
            file_info.get_absolute_path(&config).unwrap(),
