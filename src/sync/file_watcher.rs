use std::{collections::HashMap, path::Path, path::PathBuf, sync::Arc, thread, time::Duration};
...
impl FileWatcher {
    pub fn new(handler: NodeHandler<CarrierEvent>, config: Arc<Config>) -> crate::Result<Self> {
        ...
        let file_watcher = FileWatcher { notify_watcher };
    ...
    fn start_event_processing(
        ...
        thread::spawn(move || loop {
            match notify_events_receiver.recv() {
                Ok(event) => {
                    if let Some(event) = map_to_carrier_event(event, &config.paths) {
